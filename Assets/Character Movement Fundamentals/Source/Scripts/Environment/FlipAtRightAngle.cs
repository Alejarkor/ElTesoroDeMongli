...
namespace CMF
{
	...
	public class FlipAtRightAngle : MonoBehaviour {
		...
		{
			if(col.GetComponent<MongliWallkerController>() == null)
				return;

			SwitchDirection(tr.forward, col.GetComponent<MongliWallkerController>());
		}

		void SwitchDirection(Vector3 _newUpDirection, MongliWallkerController _controller)
		{
