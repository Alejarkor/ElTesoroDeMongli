...
public class MongliWallkerController : MongliCharacterController
{
    //Events;
    ...
    //ESTO SE DEBE EJECUTAR EN EL UPDATE
    void UpdateJump(bool jumpInput)
    {
        HandleJumpKeyInput(jumpInput);
    }

    //Handle jump booleans for later use in FixedUpdate;
    void HandleJumpKeyInput(bool jumpInput)
    {
        //bool _newJumpKeyPressedState = IsJumpKeyPressed();

        if (jumpKeyIsPressed == false && jumpInput == true)
            jumpKeyWasPressed = true;

        if (jumpKeyIsPressed == true && jumpInput == false)
        {
            jumpKeyWasLetGo = true;
            jumpInputIsLocked = false;
        }

        jumpKeyIsPressed = jumpInput;
    }


    //ESTO SE DEBE EJECUTAR EN EL FIXEDUPDATE
    void UpdateMovement(Vector2 moveInput)
    {
        ControllerUpdate(moveInput);
    }

    //Update controller;
    //This function must be called every fixed update, in order for the controller to work correctly;
    void ControllerUpdate(Vector2 moveInput)
    {
        ...
        if (currentControllerState == ControllerState.Grounded)
            _velocity = CalculateMovementVelocity(moveInput);
        ...
        savedMovementVelocity = CalculateMovementVelocity(moveInput);
    ...
    protected virtual Vector3 CalculateMovementDirection(Vector2 moveInput)
    {
        ...
        //{
        //	_velocity += tr.right * dir.x;
        //	_velocity += tr.forward * dir.y;
        //}
        //else
        //{
        ...
        //}
        _velocity += tr.right * moveInput.x;
    ...
    protected virtual Vector3 CalculateMovementVelocity(Vector2 moveInput)
    {
        //Calculate (normalized) movement direction;
        Vector3 _velocity = CalculateMovementDirection(moveInput);
    ...
    void HandleMomentum(Vector2 moveInput)
    {
        ...
        if (!IsGrounded())
        {
            Vector3 _movementVelocity = CalculateMovementVelocity(moveInput);
        ...
        if (currentControllerState == ControllerState.Sliding)
        {
            ...
            //Calculate movement velocity;
            Vector3 _slopeMovementVelocity = CalculateMovementVelocity(moveInput);
